...
test({ id: "O7", desc: "§23 default (no v0.10 flags) is unchanged: writer runs, no accept/chain markers", bin: "harness", args: ["q", "--local", "--format", "json"],
  check: (r) => { const j = JSON.parse(r.stdout); return r.status===0 && j.fusionix.accepted_on_consensus===undefined && j.fusionix.analysis!==undefined && inc(j.choices[0].message.content, "FINAL ANSWER"); } });
